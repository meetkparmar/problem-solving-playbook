...
fun main() {
    val solution = MergeSortedArraySolution()

    val testCases = listOf(
        MergeSortedArrayInputs(intArrayOf(1,2,3,0, 0, 0), 3, intArrayOf(2, 5, 6), 3),
        MergeSortedArrayInputs(intArrayOf(0), 0, intArrayOf(1), 1)
    )

    for (test in testCases) {
        val result = solution.merge(test.nums1, test.m, test.nums2, test.m)
        println("Input: Nums1: [${test.nums1.joinToString()}], Nums2: [${test.nums2.joinToString()}], m: ${test.m}, n: ${test.n}")
        println("-------------")
...
data class MergeSortedArrayInputs(
    val nums1: IntArray,
    val m: Int,
    val nums2: IntArray,
    val n: Int,
